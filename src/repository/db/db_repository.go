...
import (
	"github.com/cesarnono/bookstore_oauth-api/src/client/cassandra"
...
func (r *dbRepository) Create(at access_token.AccessToken) *rest_errors.RestErr {
	if err := cassandra.GetSession().Query(queryCreateAccessToken,
		...
		return rest_errors.NewInternalServerError("error when trying to save access token in database")
	}
	return nil
}
...
func (r *dbRepository) UpdateExpirationTime(at access_token.AccessToken) *rest_errors.RestErr {
	if err := cassandra.GetSession().Query(queryUpdateExpires,
		...
		return rest_errors.NewInternalServerError("error when trying to update current resource")
	}
	return nil
}
